...
- **Worktree dev setup for this project**: The main worktree is at `/Users/ankit/src/mail-app/`. Copy `.env` from the main worktree — it holds the build-time OAuth client (`MAIN_VITE_GOOGLE_CLIENT_*`), `ANTHROPIC_API_KEY`, `EXOEMAILTEST_EMAIL`, and the `EXOEMAILTEST_*` test-account credentials (client id/secret/refresh token). Dev signs in as the dedicated test account only — never the user's real inbox. **All env-loading scripts read `.env.local` first then fall back to `.env`**, so `.env` alone is sufficient. `.env.local` is supported as an optional per-worktree override but not required — there's no security difference (both are gitignored). On a fresh worktree, `.dev-data/` starts empty; run `node scripts/setup-dev-data.mjs` to materialize tokens from the refresh token, then `npm run dev`. Real-account state from `~/Library/Application Support/exo/` is **NOT** auto-copied into `.dev-data/` (that bootstrap was removed in May 2026). If you need realistic test data, run `node scripts/seed-test-inbox.mjs` once the test-account refresh token is present in `.env` (or `.env.local`).
