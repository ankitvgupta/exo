...
- **Fresh worktrees need `.dev-data/tokens.json`**: On a fresh worktree, `.dev-data/` starts empty. Before running anything that boots the app in real (non-demo) mode — `npm run dev`, the `real-gmail` Playwright project, or `npm run pre-pr` (which includes real-gmail) — run `node scripts/setup-dev-data.mjs`. It exchanges the refresh token in `.env.local` for a fresh access token and writes `.dev-data/credentials.json` + `.dev-data/tokens.json` non-interactively. Without this, `real-gmail:cached` times out waiting for the app to render because it boots into an unauthenticated state. Do NOT instead try to run interactive OAuth — `setup-dev-data.mjs` is the automation-friendly path. Real-account state from `~/Library/Application Support/exo/` is **NOT** auto-copied into `.dev-data/` (that bootstrap was removed in May 2026). If you also need realistic test inbox data, run `node scripts/seed-test-inbox.mjs` after `setup-dev-data.mjs`.
