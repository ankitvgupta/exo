...
- **🚨 As soon as local pre-pr finishes PASS and uploads the report to the PR body, rerun the `Verify pre-pr report` GitHub check — unprompted, every time, no exceptions.** Do not wait for me to ask. Do not assume it'll be green just because the local run passed. The CI job almost always raced ahead and failed against the empty PR body; rerunning it against the now-uploaded marker is the only way to turn it green. Required steps, in order:
  1. Confirm the marker is in the PR body: `gh pr view <PR#> --json body --jq .body | grep PRE-PR-REPORT` (must print the marker line).
  2. Find the failed `Verify pre-pr report` run: `gh pr checks <PR#>` or `gh run list --branch <branch> --workflow ...`.
  3. Rerun the failed job **immediately**: `gh run rerun <run-id> --failed`. Then re-check with `gh pr checks <PR#>` until just that job reports green (or surface any genuine failure).
- **🚨 Do NOT wait for the rest of CI to finish before kicking the `Verify pre-pr report` rerun.** As soon as the marker is in the PR body, fire `gh run rerun <run-id> --failed` for that specific job. Other CI jobs (lint, type-check, unit tests, e2e, etc.) are independent and will keep running in parallel — they don't gate the rerun, and waiting for them just delays the green check by minutes for no reason. Never say things like "waiting on the rest of CI to finish before I can rerun the racey Verify pre-pr report job" — that's wrong; rerun immediately, then poll everything else separately.
- Treat the full sequence as one atomic unit: `gh pr create` → `npm run pre-pr` → confirm marker in PR body → **immediately** `gh run rerun <run-id> --failed` for `Verify pre-pr report` (don't wait on other CI jobs) → `gh pr checks <PR#>` until green. Stopping at "local PASS" or at "CI is red but local was green" is the same delegation failure as stopping at `gh pr create`. Own the green check.
