...
- **Always run `npm run pre-pr` (full, no `--quick`) immediately after `gh pr create` succeeds — unprompted, every PR, no exceptions.** `gh pr create` and `npm run pre-pr` are one sequence; do not hand control back to the user, announce the PR as ready, or move on to other work without kicking it off first.
  ...
  - Run pre-pr in the foreground or via the active agent's native background/session support yourself; never tell the user "you should run pre-pr now."
...
- **Local pre-pr PASS does not mean CI is green. Always run `gh pr checks <PR#>` after the local pre-pr finishes.** The local run adds the `<!-- PRE-PR-REPORT-START -->` marker + verdict to the PR body at the end. CI's `Verify pre-pr report` job can race the PR body update and fail if it runs before pre-pr finishes.
  ...
  - Treat the sequence as: `gh pr create` → `npm run pre-pr` → `gh pr checks` → rerun `Verify pre-pr report` if it raced.
...
- **Attach screenshots to PRs with visual changes.** Any PR that touches UI — color/style tweaks, new components, layout changes, new UX flows, new buttons/panels — must include screenshots in the PR body so reviewers can see what to expect without checking out the branch. Capture each meaningful state. Use the electron-devtools-testing / browser tooling because the app is Electron. Non-visual PRs do not need screenshots.
...
- Do NOT copy `.claude/`, `.codex/`, `skills/`, `AGENTS.md`, or `CLAUDE.md` manually between worktrees — those are tracked by git and will already be in the worktree.
...
- **Fresh worktrees need two prereqs before `npm run pre-pr` or any `real-gmail` Playwright run**:
  1. `npm run build` — the `real-gmail` project launches `out/main/index.js` directly, and the main process loads the renderer from `out/renderer/` when `ELECTRON_RENDERER_URL` is unset. The agentic-verify phase of pre-pr runs `electron-vite dev`, which serves the renderer from Vite memory only, so `out/renderer/` is never written to disk. Without a real build, the renderer fails to load and the test times out waiting on `text=Exo`.
  2. `node scripts/setup-dev-data.mjs` — `.dev-data/` starts empty on fresh worktrees. This script exchanges `EXOEMAILTEST_REFRESH_TOKEN` from `.env.local` for a fresh access token and writes `.dev-data/credentials.json` + `.dev-data/tokens.json` non-interactively. Do not run `npm run dev` to OAuth interactively for agent setup.
  Both are idempotent and cheap. If you also need realistic test inbox data, run `node scripts/seed-test-inbox.mjs` after `setup-dev-data.mjs`.
...
## Bash commands
- This is absolutely critical: do not use syntax like `2>&1 &` to run commands in the background. Use the agent's native background/session support instead so the process can be tracked and stopped cleanly.
...
- Do NOT chain independent commands with `;` or `&&` in a single shell call. Use separate calls for independent commands so outputs stay readable and tool policies remain precise.

## SQLite
- When only reading from a SQLite database, use `sqlite3 -readonly` to open it.

## GitHub API
When you need to interact with GitHub (PRs, issues, checks, API calls), use the `gh` CLI. See `skills/github-cli/SKILL.md` for environment-specific setup — local Mac has `gh` pre-authenticated; hosted agents may need `gh` installed and may need the `--repo` flag when remotes go through a proxy.

## Tooling
This machine is configured via the `$BOOTSTRAP` directory.

## Shared Agent Skills
Shared skills live under `skills/`. Claude and Codex entrypoints in `.claude/skills/` and `.codex/skills/` are symlinks to the same folders; edit the root `skills/` content, not the symlink copies. When a user names one of these project skills, read `skills/<name>/SKILL.md` before acting.

Available shared skills:
- `clear-caches` — clear dev database caches for sender lookup, analysis, and draft testing.
- `electron-devtools-testing` — launch Electron with CDP and test it through the chrome-devtools MCP.
- `gh-local` — run GitHub Actions workflows locally with `act`.
- `github-cli` — environment-specific `gh` setup and usage.
- `reviewloop` — iterate on PR review bot comments and CI.
- `take-screenshots` — capture Electron workflow screenshots with Playwright.
